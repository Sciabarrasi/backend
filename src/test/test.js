...
const generateProd = () => {
    ...
    return { title: faker.commerce.product(), price: faker.commerce.price(), thumbnail: faker.image.abstract(), category: category };
};

describe('test all endpoint', () => {
    describe('GET ALL', () => {
        it('deberia responder con status 200 y ser array', async () => {
            const res = await request.get('/prods');
    ...
    describe('GET ONE', () => {
        it('deberia responder con status 200 y ser objeto', async () => {
            const res = await request.get('/prods/6413eac96d9948444e8f7748');
            expect(res.status).to.eql(200);
    ...
    //         const post = generateProd();
    //         const res = await request.post('/prods').send(post);
    ...
    describe('DEL ONE', function () {
        it('should respond 200', async () => {
            const res = await request.del('/prods/6413eac96d9948444e8f7748');
            expect(res.status).to.eql(200);
        });
    });
    ...
    //     it('should respond 200', async () => {
    //         const res = await request.del('/prods');
    //         expect(res.status).to.eql(200);
    //     });
    // });
});